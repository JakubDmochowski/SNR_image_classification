...
def main():
  ...
  parser.add_argument('-d', '--directory', type=str, help='input images directory', default=f"{os.getcwd()}/datasets/Animals-10")
  parser.add_argument('-o', '--output', type=str, help='output directory', default=f"{os.getcwd()}/datasets/animals10")
  parser.add_argument('-s', '--split', type=str, help='train/test/valid split, syntax <train>/<test>/<validation> in percentage. i.e. 60/20/20', default="70/15/15")
  ...
  for idx, c in enumerate(os.listdir(args.directory)):
    ...
    classCount = len(images)
    for imidx, filename in enumerate(images):
      row = [f"{c}/{filename}", idx + 1]
      if (imidx < classCount * splits["train"] / 100):
        train_writer.writerow(row)
      elif (imidx < classCount * (splits["train"] + splits["test"]) / 100):
